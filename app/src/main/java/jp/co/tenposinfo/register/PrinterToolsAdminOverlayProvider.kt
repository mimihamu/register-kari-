...
/**
 * 既存の各種設定画面を大きく組み替えず、責任者認証付きプリンター運用ハブへの導線を追加する。
 * ハブ側でも責任者PINを要求するため、設定画面の認証前にボタンが見えても機能を迂回できない。
 */
class PrinterToolsAdminOverlayProvider : ContentProvider() {
    ...
    private fun attachButton(activity: Activity) {
        ...
        val button = Button(activity).apply {
            ...
            backgroundTintList = ColorStateList.valueOf(Color.rgb(25, 118, 185))
            stateListAnimator = null
            elevation = dp(6).toFloat()
            contentDescription = "責任者用プリンター診断、通知設定、連続印刷試験を開く"
            setOnClickListener {
                activity.startActivity(Intent(activity, PrinterToolsHubActivity::class.java))
            }
        }
    ...
    private companion object {
        const val BUTTON_TAG = "tsuguregi_printer_tools_admin_button"
    }
}
