...
object PrinterPaperWidthTestV136 {
    const val SCR720_SAMPLE_REGISTRATION_NUMBER = "T1234567890123"
    ...
    fun buildAll(paper: ReceiptPaper, generatedAt: String): String = buildString {
        ...
        append('\n')
        append(scr720DiagnosticControlText())
        append('\n')
    ...
    /**
     * SCR-720 の実機テスト印刷で、文字・登録番号・画像を同一ジョブに含める。
     * QR は [PrinterProfilePrintabilityV136.diagnosticControlText]、カットは
     * [PrinterCommandEncoder.encodeText] の appendCut=true 側で付与されるため、
     * ここでは重複して生成しない。
     *
     * ラスタデータは 0x55 / 0x2A のみを使用し、制御コードを含め全バイトを
     * ASCII 範囲内に限定する。これにより、既存の文字コードエンコーダを
     * 通過しても ESC/POS バイト列が変化しない。
     */
    fun scr720DiagnosticControlText(): String = buildString {
        append("[SCR-720 テスト印刷]\n")
        append("文字: つぐレジ SCR-720 印字確認\n")
        append("登録番号: $SCR720_SAMPLE_REGISTRATION_NUMBER\n")
        append("画像: 8x8 チェッカーパターン\n")
        append(asControlText(scr720DiagnosticRaster()))
        append('\n')
    }

    fun scr720DiagnosticRaster(): ByteArray {
    ...
    private fun asControlText(bytes: ByteArray): String =
        bytes.map { byte -> (byte.toInt() and 0xFF).toChar() }.joinToString(separator = "")
}