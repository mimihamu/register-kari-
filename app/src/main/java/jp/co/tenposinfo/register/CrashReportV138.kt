...
/** v2.5 ERR-002 / APP-CRASH-001: privacy-safe uncaught-crash persistence and post-restart export. */
object CrashReportRuntimeV138 {
    ...
    @Volatile
    private var currentScreen = "PROCESS_START"

    @Volatile
    private var currentOperation = "PROCESS_START"
    ...
    private fun exceptionJson(error: Throwable): JSONObject {
        ...
        while (current != null && depth < 8) {
            ...
            chain.put(
                JSONObject()
                    .put("type", safeToken(current.javaClass.name, "Throwable"))
                    .put("stack", frames),
            )
        ...
        return JSONObject()
            .put("type", safeToken(error.javaClass.name, "Throwable"))
            .put("causeChain", chain)
    }
