...
class V043DriveConnectionTest {
    ...
    fun inspectorUiDocsAndWorkflowStayConnected() {
        ...
        assertTrue(screen.contains("Google Driveアプリにログイン"))
        assertTrue(screen.contains("DriveConnectionPolicy.shouldAutoImport"))

        assertTrue(registerBuild.contains("versionCode = 73"))
        assertTrue(registerBuild.contains("versionName = \"0.43.0-dev.1\""))
        assertTrue(plusBuild.contains("versionCode = 5"))
        assertTrue(plusBuild.contains("versionName = \"0.5.0-dev.1\""))
        assertTrue(workflow.contains("V043DriveConnectionTest.kt"))
        assertTrue(workflow.contains("TSUGUREGI_PLUS_v0.5.0_dev1_drive_connection_debug.apk"))
